...
##Loading the CSV
trainingData <- read.csv(url(training), na.strings=c("NA","#DIV/0!",""))
testingData <- read.csv(url(testing), na.strings=c("NA","#DIV/0!",""))
```
...
```{r prediction_rndF}
set.seed(12345)
modFitRndForest <- randomForest(classe ~ ., data=myTraining)
...
```{r prediction_final}
predictionRndForest <- predict(modFitRndForest, testing, type = "class")
predictionRndForest
```
